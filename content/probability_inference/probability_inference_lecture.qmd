...
description: "Probability distributions, the standard normal and t-distributions, Z-scores, confidence intervals, one- and two-tailed hypothesis testing, p-values, and Type I/II error — on the Arctic grayling length dataset."
...
[📥 Download the data](data/gray_I3_I8.csv){.btn .btn-primary download="gray_I3_I8.csv"}
:::
...
#| fig-height: 4
#| fig-width: 3.6
#| fig-alt: "Dodged histogram of Arctic grayling length in millimeters, colored by lake (I3 and I8), showing the distribution of fish lengths for each lake."
...
- We will focus on the **standard normal distribution** and **Student's t-distribution**

::: callout-note
**📖 Reference**

Gotelli & Ellison, *A Primer of Ecological Statistics*, Ch. 2 — Random Variables and Probability Distributions.\
Whitlock & Schluter, *Analysis of Biological Data*, Ch. 5 — Probability, and Ch. 10 — The Normal Distribution.
:::
::::
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "The dodged histogram of Arctic grayling length by lake, repeated as context for introducing probability distributions."
grayling_i3i8_plot
```
:::
::::::
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "Standard normal density curve with vertical lines and labels marking -3 through 3 standard deviations from the mean."
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "Histogram of z-scores for Lake I3 grayling lengths, showing the shape of the standardized length distribution."
z_fish_plot <- i3_df %>%
...
Proportion within 1 standard deviation = **count** the observations whose absolute z-score is ≤ 1, then divide by the number in the sample.

(That is exactly what `sum(abs(z) <= 1)` does: `abs(z) <= 1` is `TRUE`/`FALSE` for each fish, and summing `TRUE`s counts them.)
...
## Standard normal distribution - Fish Data
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "Histogram of Lake I3 grayling length in millimeters, with a red vertical line and label marking a length of 266 mm."
...
i3_mean_plot <- i3_df%>% 
  ...
  labs(x = "Length (mm)", y = "Count" , caption = "data from I3")+
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "Histogram of z-scores for Lake I3 grayling lengths, with vertical lines marking the mean and the ±1, ±2, and ±3 standard deviation boundaries, illustrating the z-distribution."
...
Whitlock & Schluter, Ch. 10 — The Normal Distribution, on standardizing a value and reading the standard normal table.
:::
:::
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "The Lake I3 z-distribution histogram repeated, shown in the context of using z-tables to look up areas under the standard normal curve."
i3_z_plot
```
:::
:::::
...
So let's do this for a fish that is 300mm long and guess the probability of catching something larger
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "The Lake I3 z-distribution histogram repeated, shown alongside the worked z-score calculation for a 300 mm fish."
i3_z_plot
...
Now work it backwards: pick a percentage first, and find the length it corresponds to. Why might you want to do it in that direction?
:::

::: {.column width="40%"}
![Standard normal (Z) table giving the cumulative area to the left of z for z-values from 0.0 to 3.4 in rows and hundredths in columns, used to look up the proportion of a normal distribution below a given z-score.](images/clipboard-1995791111.png){width="3.96in" height="3.82in"}
:::
:::::
...
- `pnorm(z)` — cumulative probability (area to the left)
- `qnorm(p)` — the z-value for a given probability
- `dnorm(z)` — the probability density at z
:::
...
::::: columns
::: {.column width="60%"}
Let's say we are interested in knowing at what point from I3 it is not likely to catch a larger fish?
...
Whitlock & Schluter, Ch. 13 — Handling Violations of Assumptions, on checking normality and what to do when it fails.
:::
:::
...
# theory vs reality out in the tail
...
Whitlock & Schluter, Ch. 4 — Estimating with Uncertainty, is the chapter that separates these two ideas.
:::
:::
...
Whitlock & Schluter, Ch. 11 — Inference for a Normal Population, introduces the t-distribution for exactly this situation.
:::
:::

![Vintage photo of William Sealy Gosset (the statistician 'Student' who derived the t-distribution while working at the Guinness brewery), edited with a speech bubble saying 'Cheers!' and a pint of Guinness beer beside him, a humorous nod to why the t-test is called 'Student's t-test.'](images/clipboard-1052237789.png){width="2.49in" height="3.20in"}
::::
...
Whitlock & Schluter, Ch. 11 — Inference for a Normal Population.\
Gotelli & Ellison, Ch. 3 — Summary Statistics, on the standard error that sits inside every t.
:::
:::
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "Overlaid density curves comparing the standard normal distribution (dashed black) to t-distributions with 3, 10, and 30 degrees of freedom, showing the t-distribution's heavier tails at low degrees of freedom converging toward the normal curve as df increases."
library(ggplot2)
library(dplyr)
...
::: {.column width="40%"}
![Student's t critical-value table with one-tail and two-tail probabilities across the columns and degrees of freedom down the rows, used to look up the t-value for a chosen confidence level and sample size.](images/clipboard-3203878802.png){width="3.96in" height="4.08in"}
:::
:::::
...
::: {.column width="40%"}
![The same Student's t critical-value table repeated, with probabilities along the top and degrees of freedom down the left column, illustrating how to read off a critical t-value for a given df and confidence level.](images/clipboard-3203878802.png){width="3.96in" height="4.27in"}
:::
:::::
...
![Bell-shaped t-distribution curve with the area to the left of t = 1.328 shaded, illustrating a one-tailed cutoff.](images/clipboard-1822465473.png){width="1.88in" height="1.56in"}
:::

::: {.column width="40%"}
![Student's t critical-value table with the one-tail 0.10 column and df = 19 row highlighted, intersecting at the critical value 1.328, the value used for a one-tailed test with n = 20.](images/clipboard-641796945.png){width="3.96in" height="4.22in"}
:::
:::::
...
![Bell-shaped t-distribution curve with both tails shaded beyond t = -1.729 and t = 1.729, illustrating a two-tailed cutoff.](images/clipboard-2234740159.png){width="2.88in" height="2.08in"}
:::

::: {.column width="40%"}
![Student's t critical-value table with the two-tail 0.10 column and df = 19 row highlighted, intersecting at the critical value 1.729, the value used for a two-tailed test with n = 20.](images/clipboard-1734806681.png){width="3.96in" height="4.83in"}
:::
:::::
...
Whitlock & Schluter, Ch. 4 — Estimating with Uncertainty (what a confidence interval means), and Ch. 11 — Inference for a Normal Population (why the multiplier is *t*, not 1.96).
:::
:::

::: {.column width="40%"}
![Student's t critical-value table with the two-tail 0.05 column and df = 8 row highlighted, intersecting at the critical value 2.306, the value used to compute the 95% confidence interval in this worked example.](images/clipboard-3959630930.png){width="3.96in" height="3.79in"}
:::
:::::
...
#| include: false
#| paged-print: false
set.seed(42)   # so the numbers on this slide are the same every render
...
## Intro to Hypothesis Testing (One-Tailed) — The Numbers
...
\
let's test if our sample mean of 320 is larger than 285 or not? Essentially we are looking at the confidence intervals!!! But we are only interested if it is larger

::: callout-note
**📖 Reference**

Gotelli & Ellison, Ch. 4 — Framing and Testing Hypotheses.\
Whitlock & Schluter, Ch. 6 — Hypothesis Testing, including one-tailed vs. two-tailed tests.
:::
::::
...
measurement_plot_1t <- ggplot(measurement_data_1t, aes(x = x, y = y)) +
  ...
  geom_vline(xintercept = sample_mean_1t, linetype = "solid", color = "green", linewidth = 1.2) +
  geom_vline(xintercept = hypothesized_mean_1t, linetype = "solid", color = "blue", linewidth = 1.2) +
...
## Intro to Hypothesis Testing (One-Tailed) — The Picture
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "One-tailed t-test rejection region plot on the measurement scale, with a red shaded rejection region above the critical value, a blue vertical line at the hypothesized mean (285), and a green vertical line at the observed sample mean (320), which falls inside the rejection region."
...
let's test if our sample mean of 320 is equal to 270 or not? Essentially we are looking at the confidence intervals!!!\

::: callout-note
**📖 Reference**

Whitlock & Schluter, Ch. 6 — Hypothesis Testing, on why a two-tailed test is the default and what it costs to go one-tailed.
:::
:::
...
ht2_tcrit <- qt(1 - ht2_alpha/2, ht2_df)

# Create measurement scale data
ht2_x <- seq(200, 380, length.out = 1000)
ht2_se <- ht2_sd / sqrt(ht2_n)

# Calculate probability density values
ht2_y <- dt((ht2_x - ht2_mu0) / ht2_se, ht2_df) * (1 / ht2_se)
...
ht2_cv_hi <- ht2_mu0 + ht2_tcrit * ht2_se
ht2_cv_lo <- ht2_mu0 - ht2_tcrit * ht2_se

# Create rejection regions
ht2_rej_hi <- data.frame(
  x = seq(ht2_cv_hi, max(ht2_x), length.out = 100),
  y = dt((seq(ht2_cv_hi, max(ht2_x), length.out = 100) - ht2_mu0) / ht2_se, ht2_df) * (1 / ht2_se)
)

ht2_rej_lo <- data.frame(
  x = seq(min(ht2_x), ht2_cv_lo, length.out = 100),
  y = dt((seq(min(ht2_x), ht2_cv_lo, length.out = 100) - ht2_mu0) / ht2_se, ht2_df) * (1 / ht2_se)
)

# Calculate t-statistic
ht2_tstat <- (ht2_mean - ht2_mu0) / ht2_se

# Create the measurement scale plot
ht2_plot <- ggplot(ht2_data, aes(x = x, y = y)) +
  geom_line(color = "blue", linewidth = 1) +
  geom_area(data = ht2_rej_hi, aes(x = x, y = y), 
            fill = "red", alpha = 0.3, inherit.aes = FALSE) +
  geom_area(data = ht2_rej_lo, aes(x = x, y = y), 
            fill = "red", alpha = 0.3, inherit.aes = FALSE) +
  geom_vline(xintercept = ht2_cv_hi, linetype = "dashed", color = "red") +
  geom_vline(xintercept = ht2_cv_lo, linetype = "dashed", color = "red") +
  geom_vline(xintercept = ht2_mean, linetype = "solid", color = "green", linewidth = 1.2) +
  geom_vline(xintercept = ht2_mu0, linetype = "solid", color = "blue", linewidth = 1.2) +
  annotate("text", x = ht2_cv_hi + 8, y = max(ht2_y) * 0.6, 
           label = "Rejection\nRegion", color = "red", size = 3.5) +
  annotate("text", x = ht2_cv_lo - 8, y = max(ht2_y) * 0.6,  
           label = "Rejection\nRegion", color = "red", size = 3.5) +
  annotate("text", x = ht2_mean + 5, y = max(ht2_y) * 0.8, 
           label = paste("Sample Mean =", ht2_mean), color = "green", size = 3.5) +
  annotate("text", x = ht2_mu0 - 5, y = max(ht2_y) * 0.9, 
           label = paste("Ho: μ =", ht2_mu0), color = "blue", size = 3.5) +
  annotate("text", x = 230, y = max(ht2_y) * 0.3,
           label = paste("t =", round(ht2_tstat, 3)), size = 3.5) +
  annotate("text", x = 230, y = max(ht2_y) * 0.2,
           label = paste("Critical t = ±", round(ht2_tcrit, 3)), size = 3.5) +
  labs(title = "Two-Tailed t-Test (Original Scale)",
       subtitle = paste0("Testing Ho: μ = ", ht2_mu0, " vs H1: μ ≠ ", ht2_mu0, " (α = ", ht2_alpha, ", df = ", ht2_df, ")"),
       x = "Measurement Value",
       y = "Probability Density") +
  theme_minimal(base_size = 9) +
  theme(plot.title = element_text(size = 14, face = "bold"),
...
cat("Sample mean:", ht2_mean, "\n")
cat("Hypothesized mean:", ht2_mu0, "\n")
cat("Standard error:", round(ht2_se, 3), "\n")
cat("t-statistic:", round(ht2_tstat, 3), "\n")
cat("Critical t-value (±):", round(ht2_tcrit, 3), "\n")
cat("Critical values:", round(ht2_cv_lo, 2), "to", round(ht2_cv_hi, 2), "\n")

# Determine if we reject H0
if(abs(ht2_tstat) > ht2_tcrit) {
...
## Hypothesis Testing (Two-Tailed) — The Picture
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "Two-tailed t-test rejection region plot on the measurement scale, with red shaded rejection regions in both tails beyond the critical values, a blue vertical line at the hypothesized mean (270), and a green vertical line at the observed sample mean (320), which falls in the upper rejection region."
...
- so report `p < 0.001` rather than `p = 1.2e-16`, and always report the effect size alongside it

::: callout-note
**📖 Reference**

Whitlock & Schluter, Ch. 6 — Hypothesis Testing.\
Gotelli & Ellison, Ch. 4 — Framing and Testing Hypotheses, "Statistical Significance and P-Values," covers these misinterpretations directly.
:::
:::
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "Standard normal density curve with a red dashed vertical line marking an observed test statistic (z = 2.1) and the area under the curve beyond it shaded blue to represent the p-value."
...
Whitlock & Schluter, Ch. 6 — Hypothesis Testing, and Gotelli & Ellison, Ch. 4 — "Errors in Hypothesis Testing." We return to both in Power and Type I and II Error.
:::
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "Overlapping density curves for a null distribution (black) centered at 0 and an alternative distribution (green) centered at 3, with the Type I error region shaded red beyond the critical value under the null curve, and the Type II error region shaded blue under the alternative curve below the critical value."
# Create a visualization of Type I and Type II errors
x <- seq(-4, 8, length.out = 1000)
null_y <- dnorm(x, mean = 0, sd = 1)
# same SD as the null - both are the sampling distribution of the SAME
# statistic, just shifted by the effect size
...
- Red shaded area (Type I Error): probability of rejecting Ho when it's actually true
  - area under the null distribution (black curve) to the right of the critical value (set by α = 0.05)
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "The Type I and Type II error density-curve plot repeated, shown alongside the explanation of what the red and blue shaded regions mean."
...
3.  **Confidence intervals** express uncertainty
    ...
    - t ≈ 1.96 only once n is large — with n = 10 it is 2.26
...
#| fig-width: 3.6
#| paged-print: false
#| fig-alt: "The Type I and Type II error density-curve plot repeated once more as a closing summary visual."
error_plot
```
:::
:::::
...
- **Whitlock & Schluter**, *The Analysis of Biological Data* — **Ch. 5**, Probability; **Ch. 10**, The Normal Distribution; **Ch. 6**, Hypothesis Testing. If you read only one, read Ch. 6
- **Gotelli & Ellison**, *A Primer of Ecological Statistics* — **Ch. 2**, Random Variables and Probability Distributions, and **Ch. 4**, Framing and Testing Hypotheses

**Where each piece of today lives:**

| Today's topic | Read |
|---------------|------|
| Probability distributions | W&S Ch. 5; Gotelli Ch. 2 |
...
| Type I / Type II error, power | W&S Ch. 6; Gotelli Ch. 4 |
...
Everything today was the machinery. Next module we point it at a real comparison between two groups, and Whitlock & Schluter **Ch. 11** and **Ch. 12** become the chapters to have read.
:::

::: callout-note
**📖 R for Data Science (2e)**

Little new R today — but the plots in this lecture are the same grammar as ever:

- [§9 — Layers](https://r4ds.hadley.nz/layers.html)
- [§10 — Exploratory data analysis](https://r4ds.hadley.nz/EDA.html)
- [§13 — Numbers](https://r4ds.hadley.nz/numbers.html) — `mean()`, `sd()`, rounding
:::
::::
:::::::
