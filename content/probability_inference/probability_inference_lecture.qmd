...
shade_curve <- function(dens, from = -Inf, to = Inf, xlim = c(-4, 4),
  ...
  ggplot(curve_df, aes(x, y)) +
    geom_area(data = fill_df, fill = "#1F78B4", alpha = 0.35) +
...
The **normal distribution** is the one we lean on:

- Symmetric, bell-shaped, defined by exactly **two numbers**
- **μ** sets where the curve sits
- **σ** sets how wide it is
- Area under the whole curve = 1

Lake I3 grayling: **μ ≈ `r round_half_up(i3_mean, 1)` mm**, **σ ≈ `r round_half_up(i3_sd, 1)` mm**.

That is a normal curve — but it is *our* normal curve. Every dataset has a different one.
...
- Left histogram: millimetres, centred near `r round_half_up(i3_mean, 0)`
- Right histogram: z, centred at 0
- Same fish, same skew, same everything
...
```{r pi-05}
#| echo: false
#| fig-alt: "Histogram of Lake I3 grayling z-scores with vertical lines at the mean and at plus and minus one, two, and three standard deviations."
i3_z_plot <- i3_df %>%
  ggplot(aes(z_score)) +
  geom_histogram(fill = "blue", alpha = 0.7, binwidth = 0.25) +
...
::: {.column width="40%"}
![](images/clipboard-1995791111.png){fig-alt="Standard normal (z) table giving the cumulative area to the left of z, with z values from 0.0 to 3.4 down the rows and hundredths across the columns." width="3.96in"}
:::
::::::
...
tibble(x = rep(seq(-4, 4, length.out = 400), 4),
  ...
  geom_line(linewidth = 0.8) +
  scale_colour_manual(values = c("#E31A1C", "#FF7F00", "#1F78B4", "black")) +
...
::: {.column width="40%"}
![](images/clipboard-3203878802.png){fig-alt="Student's t critical-value table with one-tail and two-tail probabilities across the top and degrees of freedom down the rows." width="3.96in"}
:::
:::::
...
::: {.column width="40%"}
![](images/clipboard-3959630930.png){fig-alt="Student's t critical-value table with the two-tail 0.05 column and the df = 8 row highlighted, meeting at the critical value 2.306." width="3.96in"}
:::
:::::
...
![](images/clipboard-641796945.png){fig-alt="Student's t table with the one-tail column and df row highlighted at the critical value." width="2.6in"}
:::
:::::
...
ggplot(curve_df, aes(x, y)) +
  geom_area(data = filter(curve_df, x <= -tcrit), fill = "#1F78B4", alpha = 0.35) +
  geom_area(data = filter(curve_df, x >=  tcrit), fill = "#1F78B4", alpha = 0.35) +
  geom_line(linewidth = 0.8) +
...
![](images/clipboard-1734806681.png){fig-alt="Student's t table with the two-tail column and df row highlighted at the critical value." width="2.6in"}
:::
::::::
