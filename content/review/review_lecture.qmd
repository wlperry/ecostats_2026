...
description: "End-of-course review, worked through a full logistic regression example — fitting a model, interpreting log-odds and odds ratios, visualizing the probability curve, making predictions, and assessing model fit with pseudo-R²."
categories: [Review]
order: 20
week: 10
author: "Bill Perry"
...
```{r initial_boxplot}
#| fig-alt: "Boxplot of lizard body length in centimeters split by sexual maturity status (0 = immature, 1 = mature), showing mature lizards tend to be longer than immature ones but with overlap between the two groups."
...
#| fig-alt: "Scatterplot of observed sexual maturity (0 or 1) versus lizard body length, with a red S-shaped logistic curve overlaid showing the predicted probability of maturity rising from near 0 at small lengths to near 1 at large lengths, with the steepest rise around 25-30 cm."
...
```{r diagnostic_plot}
#| fig-alt: "Scatterplot of observed maturity (points) versus body length with a blue line showing the model's predicted probability of maturity, and a dashed gray horizontal reference line at the 50% probability threshold."
