...
::: callout-note
**🔮 Predict first:** Module 3 found `mean(length_mm)` ≈ 17.7 and a fairly small gap from the median. What shape do you expect the full distribution to have — symmetric, or skewed?
:::
...
```{r}
#| eval: false
# Typo: "wind" instead of the real value "s"
ggplot(pine_df, aes(x = n_s, y = length_mm, color = n_s)) +
  geom_point() +
  scale_color_manual(
    values = c(n = "#2c7fb8", wind = "#d95f0e")
  )
```
:::
