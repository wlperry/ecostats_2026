...
pine_labeled_plot <- pine_box_plot +
  labs(
    ...
    x        = "Side of tree (lee = n, windward = s)",
    y        = "Needle length (mm)",
    fill     = "Side",
...
✏️ **Your turn:** What's the difference between writing `color = group` inside `aes()` versus writing `color = "steelblue"` outside `aes()`? Try both and describe what changes.
...
✏️ **Your turn:** Do the lee and windward density curves look like they overlap a lot, or are they clearly separated? `________________________`
...
ggplot(pine_df, aes(x = n_s, y = length_mm, color = n_s)) +
  ...
  scale_color_manual(
    name   = "Side of tree",
    labels = c(n = "Lee (sheltered)", s = "Windward"),
...
✏️ **Your turn:** In one sentence, when do you reach for `scale_color_manual()` versus `scale_fill_manual()`? `________________________`
