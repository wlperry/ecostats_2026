...
title: "Lecture: Multivariate Statistics"
subtitle: "Multivariate intro"
description: "Introduction to multivariate analysis of ecological data — standardization, Euclidean distance, principal component analysis (PCA), loadings and scores, ordination, and PERMANOVA for testing group differences, using a Darlingtonia (cobra lily) morphology dataset."
...
## Part 1 · Why Multivariate Data? {.divider}
...
::: callout-note
Gotelli & Ellison, *A Primer of Ecological Statistics*, Ch. 12 — The Analysis of Multivariate Data, is the core reference for this lecture.
:::
::::
...
![Photo of a cobra lily (Darlingtonia californica) showing its hooded, pitcher-shaped leaves and forked, tongue-like appendages, the carnivorous plant whose morphology this lecture's PCA example analyzes.](images/clipboard-3752708543.png){width="3.49in"}
:::
::::::
...
![Vintage-style line illustration of three Darlingtonia pitcher-leaf structures with their hooded tops, tube bodies, and forked wing appendages labeled by shape, showing the plant parts behind variables like height, mouth diameter, tube diameter, and wing length/spread used in the PCA.](images/clipboard-4005369252.png){width="2.20in"}
...
[📥 Download the data](data/darlingtonia.csv){.btn .btn-primary download="darlingtonia.csv"}
...
# Load the data
darlingtonia <- read_csv("data/darlingtonia.csv")
...
#| fig-width: 3.6
#| fig-height: 3.6
#| fig-alt: "Faceted boxplots of each of the 10 raw Darlingtonia morphology and biomass variables by collection site, each panel on its own free y-axis scale, showing the variables have very different ranges."
...
```{r standardize}
#| echo: false
#| fig-alt: "Boxplots of standardized (z-scored) height, mouth diameter, and wing mass, all shown on the same scale centered around zero, illustrating that standardization puts differently-scaled variables on equal footing."
...
#| fig-width: 3.6
#| fig-height: 3.6
#| fig-alt: "Two stacked panels of boxplots by site for height and mouth diameter: raw values on different scales on top, and standardized (z-scored) values on a common scale below, showing standardization equalizes the variables' influence."
...
## Part 2 · Correlation and the Concept of PCA {.divider}
...
#| fig-width: 3.6
#| fig-height: 3.6
#| fig-alt: "Correlation matrix heatmap of the 10 Darlingtonia morphology and biomass variables, colored and hierarchically clustered, with correlation coefficients printed in each cell, showing several strongly correlated variable clusters."
...
::: {.column width="40%"}
![Scatterplot of a correlated two-variable Gaussian point cloud with two perpendicular black arrows overlaid, the longer arrow pointing along the direction of greatest spread (the first principal axis) and the shorter arrow perpendicular to it (the second), illustrating how PCA finds new axes aligned with the directions of maximum variance.](images/clipboard-2517015273.png){width="2.56in"}\
...
## Part 3 · Running PCA in R {.divider}
...
#| fig-width: 3.6
#| fig-height: 3.6
#| fig-alt: "Scree plot of percentage variance explained by each of the 10 principal components as blue bars, with a red cumulative-variance line and a dashed reference line at 70%, showing the first three components already exceed that threshold."
...
ggplot(scree_data, aes(x = PC)) +
  geom_col(aes(y = Variance), fill = "darkblue", alpha = 0.7) +
...
## Part 4 · Interpreting Loadings and Components {.divider}
...
#| fig-width: 4
#| fig-height: 3.6
#| fig-alt: "Horizontal bar chart of PC1 loadings for each of the 10 variables, colored by sign, showing most variables loading positively (darkblue) and only keel and tube diameter loading negatively (coral), consistent with PC1 representing overall plant size."
...
#| fig-width: 3.6
#| fig-height: 3.6
#| fig-alt: "Horizontal bar chart of PC2 loadings for each of the 10 variables, colored by sign, showing height and mouth diameter loading negatively and wing measurements loading positively, consistent with PC2 representing a height-versus-wing-size shape trade-off."
...
#| fig-width: 3.6
#| fig-height: 3.6
#| fig-alt: "Horizontal bar chart of PC3 loadings for each of the 10 variables, colored by sign, showing tube diameter and keel diameter with strong positive loadings and all other variables near zero, consistent with PC3 representing tube girth."
...
## Part 5 · Scores, Ordination and Biplots {.divider}
...
#| fig-width: 3.6
#| fig-height: 3.6
#| fig-alt: "Scatterplot of PC1 versus PC2 scores for each plant, colored by site with 68% confidence ellipses, showing partial separation between sites in the size-shape plane."
...
#| warning: false
#| paged-print: false
#| fig-alt: "PC1, PC2, and PC3 scores for each plant colored by site: an interactive 3D scatterplot in HTML output, or a 2D scatterplot of PC1 vs PC2 with point size mapped to PC3 in docx/pptx output."
...
if (knitr::is_html_output()) {
  ...
  pc3d_plot <- ggplot(pc_data, aes(x = PC1, y = PC2, color = site, size = PC3)) +
    geom_point(alpha = 0.7) +
    labs(title = "PC1 vs PC2 (point size = PC3)",
...
#| fig-height: 3.6
#| warning: false
#| fig-alt: "PCA biplot of PC1 and PC2 with plants as points colored by site with confidence ellipses, and arrows for each original variable showing its loading direction and strength, with most size-related arrows pointing right and wing arrows pointing up."
...
## Part 6 · Testing Group Differences {.divider}
...
#| fig-width: 3.6
#| fig-height: 3.6
#| fig-alt: "Boxplot of PC1 (overall size) scores by site with jittered points overlaid, showing the HD site's plants scoring noticeably lower (smaller) than other sites."
...
-   **TJH plants have different shapes**
    ...
    #| fig-width: 3.6
    #| fig-height: 3.6
    #| fig-alt: "Boxplot of PC2 (shape) scores by site with jittered points overlaid, showing the TJH site's plants scoring differently, consistent with taller, narrower pitchers with smaller wings."
...
#| warning: false
#| paged-print: false
#| fig-alt: "Heatmap of the Euclidean distance matrix between site centroids in PC space, colored from teal (small distance) to orange (large distance)."
...
## Part 7 · Summary and When to Use PCA {.divider}
...
#| fig-width: 3.6
#| fig-height: 3.6
#| fig-alt: "Scatterplot of PC1 versus PC2 scores with points more than 3 standard deviations from the mean highlighted in red and labeled by plant ID, used to check for multivariate outliers."
...
## Part 8 · Best Practices and Reporting {.divider}

## Practical Tips for PCA — Best Practices
...
3.  **Standardize appropriately**
    -   Use `scale. = TRUE` (note the dot) for different units
...
## Reporting PCA Results — What to Include
...
## Part 9 · PERMANOVA — Testing Group Differences {.divider}

## Alternative PCA Visualization: FactoMineR

::: panel
```{r l16-03}
#| fig-alt: "Two PCA plots from FactoMineR/factoextra: a biplot with plants colored by site, confidence ellipses, and variable arrows; and a separate individuals-only plot with plants colored by site and 95% confidence ellipses."
...
## Quick PCA Plot with ggfortify

::: panel
```{r l16-04}
#| fig-alt: "Quick PCA plot built with ggfortify::autoplot(): plants plotted on PC1 and PC2, colored by site, with variable loading arrows and labels overlaid, and normal-distribution ellipses around each site's points."
...
## PCA with vegan: rda() and Distance Metrics

::: panel
```{r l16-05}
#| fig-alt: "Two base-R vegan plots: a PCA biplot of samples and variable arrows from rda(), and an ordination plot of sites with spider lines connecting each point to its site centroid and shaded confidence ellipses around each site."
...
## Testing Group Differences: PERMANOVA

::: panel
```{r l16-06}
#| fig-alt: "PCA biplot of plants colored and grouped by site with confidence ellipses, titled with the PERMANOVA p-value testing whether site groups differ significantly in multivariate space."
# Analysis
pca_vegan <- rda(darl_matrix)

# darl_matrix is SCALED, so it contains negative values. adonis2() defaults
# to Bray-Curtis, which is undefined for negatives -- R warns "results may be
# meaningless". For standardized continuous data the right choice is Euclidean.
...
## PERMANOVA's Assumption: Equal Dispersion

::: panel
A significant PERMANOVA can mean groups have different **centroids** (what we
usually want) *or* merely different **spread**. `betadisper()` separates them.

```{r l16-06b}
#| collapse: true
# Are the groups equally variable around their own centroids?
disp <- betadisper(vegdist(darl_matrix, method = "euclidean"),
                   darlingtonia$site)
anova(disp)

# average distance to centroid, by site
round(tapply(disp$distances, darlingtonia$site, mean), 2)
```

::: callout-important
**⚠️ Read both tests together**

- **PERMANOVA significant + dispersion NOT significant** → groups genuinely sit in different places. Clean result.
- **PERMANOVA significant + dispersion ALSO significant** → the location difference may be partly or wholly a spread difference. Say so.

Here dispersion is **borderline** (F = 2.64, p = 0.055): LEH plants are noticeably more variable (mean distance to centroid 2.83, vs 1.97–2.32 elsewhere). The site effect is strong enough (R² = 0.25, p = 0.001) that it is not *only* dispersion — but an honest write-up mentions it.
:::
:::

## Ordination with Convex Hulls

::: panel
```{r l16-07}
#| fig-alt: "Ordination plot of sites with a convex hull polygon drawn around each site's points and points colored/filled by site, showing the spatial extent of each site's morphological variation."
...
## Pairwise MANOVA-Based Comparisons
...
## Comparing the Pairwise Test Results

These results come from different statistical tests being used:

**1. `pairwise.adonis2()` — PERMANOVA on subsets**

- **What it does**: For each pair, it subsets the data to just those 2 sites and runs a separate PERMANOVA.
- **Results**: All p-values ≤ 0.016 (5 of 6 are p = 0.001).
- **Interpretation**: Tests if centroids (mean positions) differ between site pairs.

**2. `pairwise.perm.manova()` — a different test entirely**

- **What it does**: Uses a MANOVA-based approach with permutations on the full distance matrix.
- **Results**: More conservative — DG vs LEH now p = 0.078 (not significant).
- **Interpretation**: Also tests centroid differences but uses a different statistical framework.

**Why the differences matter — both are Location (Centroid) tests:**
