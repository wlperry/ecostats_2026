...
``` r
# ── Part 3.4 · Exploratory Data Analysis and Effect Size ────────────────
# Write your code in the R script:
```
:::
...
``` r
# ── Part 5.1 · Statistical Power ────────────────────────────────────────
side_diff <- 1

sunny_n <- nrow(sunny_df)
shady_n <- nrow(shady_df)

sun_sd_pooled <- sqrt((var(sunny_df$needle_length_mm) * (sunny_n - 1) +
                      var(shady_df$needle_length_mm) * (shady_n - 1)) /
                      (sunny_n + shady_n - 2))

sun_effect_size <- side_diff / sun_sd_pooled
sun_deg_free <- sunny_n + shady_n - 2
sun_alpha <- 0.05
...
✏️ **Your turn:** Is the power above 80%? If our study is underpowered, what are two things we could change to increase power?
