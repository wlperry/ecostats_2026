...
- What to do when assumptions fail:
  ...
  - Permutation tests
  - The paired **Wilcoxon signed-rank** test — it ranks the *sizes* of the paired differences and then attaches their signs, so it uses more information than a plain sign test. It is the standard non-parametric partner to the paired t-test, and we ran it on our own four trees
  - And we saw its limit: with 4 pairs it cannot return a p-value below 0.125, no matter how large the effect
...
::: callout-note
**✅ Our own design, in these terms**

Eight needles from one branch are **not** independent — they share a tree, a microclimate, a genotype. Two sides of the same tree **are** dependent on each other too, but deliberately so: that is a **paired** design, and pairing turns a violation of independence into an advantage.

Non-independence you ignore costs you. Non-independence you plan for pays you.
:::
::::
...
:::: {.column width="40%"}
::: callout-important
**⚠️ There are two things called "post hoc power" — one is useful, one is circular**

**Legitimate:** "Given our n and σ, what power did we have to detect a difference *we decided in advance* was biologically meaningful?" That is a real answer about your design, and it is what the bullets on the left describe.

**Invalid:** computing power from the effect size you *observed*. Observed power is a direct mathematical re-expression of your p-value — a non-significant result *always* yields low observed power. Saying "our result was non-significant and our power was low" says the same thing twice, and is not evidence that there is no effect.

If you want to argue for no effect, use an **equivalence test** or report a confidence interval that excludes any meaningful difference.
:::
::::
:::::::
...
**We already have pilot data — our own.** Four trees, both sides of each, from Week 1. That is exactly what a pilot study is *for*: it does not settle the question, it tells you how big the real study has to be.

```{r study-design-sampling-06}
#| fig-height: 3
#| fig-width: 3
# Estimates from OUR four trees (see the Power lecture)
shady_mean  <- 14.12   # mm
sunny_mean  <- 15.21   # mm
pooled_sd   <- 1.81    # mm

effect_size <- abs(shady_mean - sunny_mean) / pooled_sd
effect_size            # Cohen's d

# How many trees per side would a proper study need?
pwr.t.test(d = effect_size,
           sig.level = 0.05,
           power = 0.8,
           type = "two.sample")
...
✏️ **Your turn:** Now re-run it as a **paired** design (`type = "paired"`) using the effect size for the *differences*, `d = 1.36`. How many trees does that need instead?
:::

::: callout-tip
**🖐 The design decision, in one comparison**

- Unpaired: **44 trees per side** = 88 trees
- Paired: **7 trees**, both sides of each

Same question, same measurement effort per tree, same α and power. The difference is entirely in **how you decide to sample** — which is what this whole lecture is about.
:::
