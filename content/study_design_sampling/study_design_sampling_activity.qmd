...
>
> **Every grey `r` box below is code you type into an R script — not into the Console.** Before you start: **File → New File → R Script**, and save it as `09_study_design_sampling.R` in your project's `scripts/` folder. Then work down the worksheet, adding each block to the bottom of that one script, in order.
>
...
``` r
# ── Part 1.1 · Setup and the Research Question ──────────────────────────
library(tidyverse)
library(pwr)
...
✏️ **Your turn:** Looking at `stream_plot`, does every stream show fish getting larger below the waterfall, or does the pattern vary stream to stream?
...
✏️ **Your turn:** Compare the p-values from `pseudo_test` and `proper_test`. Which one is smaller? Why does pretending fish are independent make the result look more significant than it should?
...
✏️ **Your turn:** According to `power_result`, how many streams do we need for 80% power?
...
``` r
# ── Part 5.1 · Post-Hoc Power Analysis ──────────────────────────────────
...
``` r
# ── Part 5.2 · Post-Hoc Power Analysis ──────────────────────────────────
...
✏️ **Your turn:** What was our actual power? Is it above or below the 80% "acceptable" threshold from lecture?

`________________________`

::: callout-important
**⚠️ Careful — this is *observed* power**

We just computed power using `observed_effect`, the effect size from **this very sample**. That number is a mathematical re-expression of the p-value you already have: a small p always gives high observed power, a large p always gives low observed power. It cannot tell you anything the p-value did not.

Use it the way we did in the Power lecture — as an **input to planning the next study**. Do not use it to argue "our result was non-significant *because* we had low power," which is circular.

✏️ **Your turn:** Redo the calculation for an effect size you decide in advance is biologically meaningful — say `d = 0.8` — instead of the observed one. That answers a real question: *could this design have detected an effect worth caring about?*

``` r
# ── Part 5.3 · Post-Hoc Power Analysis ──────────────────────────────────
# Write your code in the R script:
```
:::
...
``` r
# ── Part 5.4 · Post-Hoc Power Analysis ──────────────────────────────────
# Write your code in the R script:
```
:::
...
``` r
# ── Part 6.1 · Alternative Analysis Approaches ──────────────────────────
...
``` r
# ── Part 6.2 · Alternative Analysis Approaches ──────────────────────────
...
✏️ **Your turn:** Compare the `se_mass` values in `summary_df` (fish-level) vs. `stream_summary` (stream-level). Which is smaller, and why does that make fish-level analysis look artificially more precise?
...
``` r
# ── Part 6.3 · Alternative Analysis Approaches ──────────────────────────
# Write your code in the R script:
```
:::
...
``` r
# ── Part 7.1 · Design Your Own Study ────────────────────────────────────
fast_mean <- 75
slow_mean <- 85
sd_val <- 15
...
``` r
# ── Part 7.2 · Design Your Own Study ────────────────────────────────────
# Write your code in the R script:
```
:::
