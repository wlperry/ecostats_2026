...
>
> **Every grey `r` box below is code you type into an R script — not into the Console.** Before you start: **File → New File → R Script**, and save it as `13_two_way_anova.R` in your project's `scripts/` folder. Then work down the worksheet, adding each block to the bottom of that one script, in order.
>
...
``` r
# ── Part 2.1 · Building a Balanced Dataset ──────────────────────────────
original_n <- p_df %>%
  count(spp, sex) %>%
  arrange(spp, sex)
...
``` r
# ── Part 2.2 · Building a Balanced Dataset ──────────────────────────────
summary_df <- pb_df %>%
  group_by(spp, sex) %>%
  summarise(
    n = sum(!is.na(mass_g)),
...
✏️ **Your turn:** What was `min_n`? How many total observations were dropped to force balance?
...
``` r
# ── Part 3.1 · Fitting the Balanced Factorial ANOVA ─────────────────────
...
✏️ **Your turn:** Is the species:sex interaction significant?
...
``` r
# ── Part 4.1 · Assumptions and Diagnostics ──────────────────────────────
par(mfrow = c(2, 2))
plot(pb_model)
par(mfrow = c(1, 1))
```
...
``` r
# ── Part 4.2 · Assumptions and Diagnostics ──────────────────────────────
...
``` r
# ── Part 4.3 · Assumptions and Diagnostics ──────────────────────────────
...
``` r
# ── Part 5.1 · Estimated Marginal Means and Post-Hoc ────────────────────
...
``` r
# ── Part 5.2 · Estimated Marginal Means and Post-Hoc ────────────────────
...
``` r
# ── Part 5.3 · Estimated Marginal Means and Post-Hoc ────────────────────
...
``` r
# ── Part 5.4 · Estimated Marginal Means and Post-Hoc ────────────────────
...
``` r
# ── Part 5.5 · Estimated Marginal Means and Post-Hoc ────────────────────
...
✏️ **Your turn:** Using the compact letter display (`cldb_df`), which species/sex combinations are *not* significantly different from each other?
...
``` r
# ── Part 5.6 · Estimated Marginal Means and Post-Hoc ────────────────────
# Write your code in the R script:
```
:::
...
``` r
# ── Part 6.1 · Understanding Sums-of-Squares Types ──────────────────────
...
``` r
# ── Part 6.2 · Understanding Sums-of-Squares Types ──────────────────────
...
✏️ **Your turn:** For this *balanced* dataset, do the three SS types give the same F-values for the main effects? Is that what you'd expect?
...
``` r
# ── Part 7.1 · The Same Analysis, Unbalanced ────────────────────────────
...
``` r
# ── Part 7.2 · The Same Analysis, Unbalanced ────────────────────────────
par(mfrow = c(2, 2))
plot(pu_model)
par(mfrow = c(1, 1))
...
``` r
# ── Part 7.3 · The Same Analysis, Unbalanced ────────────────────────────
...
``` r
# ── Part 7.4 · The Same Analysis, Unbalanced ────────────────────────────
...
✏️ **Your turn:** Now compare `ss_comparison_df` from this *unbalanced* run to `ssb_comparison_df` from Part 6. Do the three SS types now disagree with each other?
...
``` r
# ── Part 7.5 · The Same Analysis, Unbalanced ────────────────────────────
# Write your code in the R script:
```
:::
