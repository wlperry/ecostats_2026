...
# This is the effect size that pairs with Type III SS, because Type III
# sums of squares do NOT add up to SS_total once a design is unbalanced.
ss_effects  <- anova_type3$`Sum Sq`[2:4]
ss_residual <- anova_type3$`Sum Sq`[5]

eta_squared_df <- data.frame(
  Effect          = c("Species", "Sex", "Interaction"),
  Partial_Eta_Sq  = round(ss_effects / (ss_effects + ss_residual), 4)
)
...
::: callout-note
**📖 Which effect size, and why**

$$\eta^2_p = \frac{SS_{effect}}{SS_{effect} + SS_{residual}}$$

**Partial** η² asks "of the variation this effect *could* explain, how much does it?" Plain η² divides by $SS_{total}$ instead.

They agree only when a design is balanced. **Type III sums of squares do not add up to $SS_{total}$ in an unbalanced design** — so dividing by their sum gives a number that is neither η² nor η²ₚ. Partial η² is well defined either way, which is why we use it throughout this lecture.
:::
::::
:::::::
...
#| warning: false
#| paged-print: false
# Same partial eta squared as the balanced model, so the two are comparable.
ss_effects_u  <- anova_type3_u$`Sum Sq`[2:4]
ss_residual_u <- anova_type3_u$`Sum Sq`[5]

eta_squared_p_df <- data.frame(
  Effect         = c("Species", "Sex", "Interaction"),
  Partial_Eta_Sq = round(ss_effects_u / (ss_effects_u + ss_residual_u), 4)
)
print("Effect Sizes (partial eta-squared, Type III):")
...
ss_sex_only <- sum((fitted(sex_only_model) -
                   grand_mean)^2)
cat(paste0("SS(Sex alone):     ",
           round(ss_sex_only, 0), "\n"))

cat(paste0("\nSum of main effects: ",
           round(ss_species_only + ss_sex_only, 0), "\n"))
cat(paste0("Actual between SS:   ",
           round(ss_between, 0), "\n"))
cat(paste0("\nDifference (design is not orthogonal): ",
           round(ss_between -
...
# Carry the species label along with its estimate rather than assuming the
# rows come back in a particular order — if the factor levels are ever
# reordered, positional indexing would silently mislabel these.
# Take the direction from the contrast label itself rather than typing it in
# -- emmeans returns "female - male", so a negative estimate means females
# are LIGHTER. Hard-coding the opposite label would flip the story.
contrast_label <- as.character(contrasts_df$contrast[1])

interaction_summary_df <- contrasts_df %>%
  transmute(Species    = as.character(species),
            Sex_Effect = round(estimate, 0))
cat(paste0("Sex effect (", contrast_label, ", grams) by species:\n"))
...
- Fitted a **factorial** model with two categorical predictors — `body_mass_g ~ species * sex`
- Separated **main effects** from the **interaction**, and saw that an interaction means "the effect of one factor depends on the level of the other"
- Used **estimated marginal means** instead of raw group averages, because with unequal cell sizes those are not the same thing
- Met **Type I, II and III sums of squares**, and saw them give identical answers on a balanced design and different answers on an unbalanced one
- Set `contr.sum` contrasts **before** asking for Type III — without that step R silently returns the wrong main-effect tests
- Reported **partial η²** as the effect size that stays well defined either way

::: callout-important
**⚠️ Read the interaction first**

If the interaction is significant, a main effect is a *average across conditions that behave differently* — often not a meaningful quantity. Interpret the interaction, then use simple effects (`emmeans(model, ~ sex | species)`) rather than reporting the main effect as if it applied everywhere.

R even warns you: *"Results may be misleading due to involvement in interactions."*
:::
::::
...
**Balance is a property of your design, not your analysis.** A balanced design makes the two factors orthogonal, so each one's sum of squares is unambiguous. Imbalance makes them share variance — and then *how* you allocate that shared variance (Type I vs III) becomes a decision you have to make and defend.
:::

::: callout-note
**Up next — Nested ANOVA**

- When one factor is nested inside another rather than crossed with it
- Subsamples, and the right error term to test against
:::
::::
:::::::
...
2.  Check the usual suspects: did you run `options(contrasts = c("contr.sum", "contr.poly"))` **before** fitting the model you pass to `Anova(type = "III")`? Are both predictors **factors**? Is it `car::Anova()` (capital A, Type II/III) rather than `anova()` (lower case, Type I)?
