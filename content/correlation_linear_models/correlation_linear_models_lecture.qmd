...
```{r correlation-linear-models-01}
#| include: false
#| message: false
#| warning: false
library(knitr)
...
:::: {.column width="40%"}
```{r correlation-linear-models-02}
#| echo: false
#| message: false
#| warning: false
#| fig-width: 3.2
...
- You *assume* a direction — X → Y — when you choose which variable goes where
- Produces an equation for prediction, with slope and intercept parameters

::: callout-warning
**⚠️ Regression does not earn you causation**

Choosing which variable to call X does not make it a cause. `lm(y ~ x)` will fit a slope to any two columns you hand it, including ones with no causal link at all.

What licenses a causal claim is the **study design** — manipulation, randomization, controls (Study Design & Sampling) — not the model you fit afterwards.
:::
::::

:::: {.column width="40%"}
```{r correlation-linear-models-03}
#| echo: false
#| message: false
#| warning: false
#| fig-height: 4
...
:::: {.column width="40%"}
```{r correlation-linear-models-04}
#| echo: false
#| fig-height: 4
#| fig-width: 3
#| message: false
...
:::: {.column width="40%"}
```{r correlation-linear-models-05}
#| echo: false
#| message: false
#| warning: false
#| fig-height: 3.4
...
:::: {.column width="40%"}
```{r correlation-linear-models-06}
#| echo: false
#| fig-height: 3.4
#| fig-width: 3.4
#| message: false
...
:::: {.column width="40%"}
```{r correlation-linear-models-07}
#| echo: false
#| fig-height: 3.4
#| fig-width: 3.4
#| message: false
...
:::: {.column width="40%"}
```{r correlation-linear-models-08}
#| echo: false
#| fig-height: 3.4
#| fig-width: 3.4
#| message: false
...
:::: {.column width="40%"}
```{r correlation-linear-models-09}
#| echo: false
#| fig-height: 3.6
#| fig-width: 3.6
#| message: false
...
:::: {.column width="40%"}
```{r correlation-linear-models-10}
#| echo: false
#| fig-height: 3.4
#| fig-width: 3.4
#| message: false
...
:::: {.column width="40%"}
```{r correlation-linear-models-11}
#| echo: false
#| message: false
#| warning: false
#| fig-height: 4
...
:::: {.column width="40%"}
```{r correlation-linear-models-12}
#| echo: false
#| message: false
#| warning: false
#| fig-height: 3.6
...
:::: {.column width="60%"}
```{r correlation-linear-models-13}
#| echo: false
#| message: false
#| warning: false
#| fig-width: 4
...
:::: {.column width="40%"}
```{r correlation-linear-models-14}
#| echo: false
#| fig-height: 3.6
#| fig-width: 3.6
#| message: false
...
:::: {.column width="40%"}
```{r correlation-linear-models-15}
#| echo: false
#| fig-height: 3.4
#| fig-width: 3.6
#| message: false
...
:::: {.column width="40%"}
```{r correlation-linear-models-16}
#| echo: false
#| fig-height: 3.4
#| fig-width: 3.6
#| message: false
...
:::: {.column width="40%"}
```{r correlation-linear-models-17}
#| echo: false
#| message: false
#| warning: false
#| fig-height: 3.4
...
:::: {.column width="40%"}
```{r correlation-linear-models-18}
#| echo: false
#| message: false
#| warning: false
#| fig-height: 3.4
...
:::: {.column width="40%"}
![Diagram of the regression line mu_yi = beta0 + beta1*xi running through X-Y space, with small normal-distribution curves drawn at two X values (x1 and x2) around the line to show that Y is assumed to be normally distributed around the true regression line at every value of X.](images/clipboard-2092034232.png){width="3.12in" height="3.22in"}
::::
:::::::
...
::::::: columns
:::: {.column width="60%"}
Every assumption check from here on is about the **residuals**, not the raw data. That is the single most common mix-up in regression.
...
::::::: columns
:::: {.column width="60%"}
**Residuals vs. fitted** is the linearity check.

**What you want:** a shapeless cloud, scattered evenly above and below the horizontal line at zero.

**What is a problem:** any *curve* in the red trend line. A U-shape or arch means the relationship is not straight, and a straight-line model is the wrong shape for the data.

The plot also doubles as a **homoscedasticity** check — if the cloud fans out or funnels in as you move right, the variance is not constant.
::::

:::: {.column width="40%"}
```{r correlation-linear-models-19}
#| echo: false
#| message: false
#| warning: false
#| paged-print: false
...
::::::: columns
:::: {.column width="60%"}
**The QQ plot** compares your residuals to what perfectly normal residuals would look like.

**What you want:** points hugging the diagonal line.

**What is a problem:** systematic curvature, or points peeling away at the ends — the tails are where non-normality shows up first, and the tails are what p-values depend on.

Note we check the residuals here, **not** `age_years` itself. The raw ages are right-skewed; that is allowed. It is the residuals that must behave.
::::

:::: {.column width="40%"}
```{r correlation-linear-models-20}
#| echo: false
#| message: false
#| warning: false
#| paged-print: false
...
::::::: columns
:::: {.column width="60%"}
The formal test to go with the QQ plot. Note what goes inside it:

``` r
shapiro.test(residuals(lion_model))
```

**`residuals(model)`** — not `lion_data$age_years`. Testing the raw response is a different (and usually wrong) question.

**H₀ is "the residuals ARE normal",** so here a **large** p-value is the good outcome — the same backwards logic as Levene's test.

::: callout-warning
**⚠️ Do not let this test overrule your eyes**

At small n it has almost no power to detect a problem; at large n it flags departures too small to matter. Read the QQ plot first and use the test to support it, not replace it.
:::
::::

:::: {.column width="40%"}
```{r correlation-linear-models-21}
#| echo: false
#| message: false
#| warning: false
#| fig-height: 3.4
...
:::: {.column width="60%"}
```{r correlation-linear-models-22}
#| echo: false
#| fig-height: 4
#| fig-width: 4
#| message: false
...
:::: {.column width="40%"}
![Table of formulas for the ordinary least squares estimates and standard errors of the slope (beta1), intercept (beta0), and error term (epsilon) in simple linear regression, each row giving the OLS estimate formula and its corresponding standard error formula.](images/clipboard-934594306.png){width="3.12in"}
::::
:::::::
...
:::: {.column width="40%"}
![Two side-by-side scatterplots (a) and (b) each with a fitted regression line and one point marked with its observed value y_i and predicted value y-hat_i, illustrating how the residual (the gap between them) varies from point to point and feeds into partitioning total variation.](images/clipboard-2817787184.png){width="3.12in"}
::::
:::::::
...
:::: {.column width="40%"}
![ANOVA table for simple linear regression listing Regression, Residual, and Total sources of variation with their sum-of-squares, degrees of freedom (1, n-2, n-1), mean-square, and expected-mean-square formulas.](images/clipboard-77063277.png){width="3.12in"}
::::
:::::::
...
:::: {.column width="40%"}
![Four scatterplots (a)-(d) with fitted regression lines of differing steepness and fit quality, each annotated with an observed point y_i, its prediction y-hat_i, and the mean y-bar, used to compare how much regression sum-of-squares and residual sum-of-squares differ between a strong fit (a, c) and a weak fit (d).](images/clipboard-217151541.png){width="3.12in"}
::::
:::::::
...
- Separated **correlation** (how tightly two variables move together) from **regression** (an equation predicting Y from X) — and saw that neither one earns you causation
- Computed and interpreted **Pearson's r**, squared it to get the variance explained, and tested it with `cor.test()`
- Switched to **Spearman's ρ** when the normality assumption failed
- Fitted a straight line with `lm()`, and read its **slope** and **intercept** as biology, not just numbers
- Checked all four regression assumptions **on the residuals** — residuals vs. fitted, QQ plot, Shapiro-Wilk
- Partitioned the variance into $SS_{regression}$ and $SS_{residual}$, and turned those into mean squares and an F-ratio

::: callout-note
**✅ The one idea to keep**

A regression line can be fitted to *any* two numeric columns. Everything that makes it trustworthy — linearity, independent observations, constant variance, normal residuals — lives in the **diagnostics**, not in the p-value.
:::
::::
...
- More than one predictor at a time
- What happens when predictors are correlated with *each other*
...
Finish the worksheet, and for your own model write one sentence interpreting the slope **in the units of the data** — "for every extra X, Y changes by ___" — not just "it was significant."
:::
::::
:::::::
...
2.  Check the usual suspects: is `lm()` written `y ~ x` (response first)? Did you pass `residuals(model)` where a model was expected, or vice versa?
