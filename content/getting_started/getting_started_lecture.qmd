---
metadata-files:
  - ../../_templates/_lectures.yml
title: "Lecture: Getting Started"
subtitle: "Asking a question, meeting R, and making your first graph"
description: "A question worth testing, a project worth organizing, and a first look at R, Positron, and ggplot — all built around one pine needle experiment."
categories: [Foundations, R, Graphing]
order: 1
week: 1
author: "Bill Perry"

format:
  html:
    downloads: [docx, pptx]
  revealjs:
    output-ext: "slides.html"
  docx:
    reference-doc: ../../ms_templates/custom-reference-compact.docx
    fig-width: 2.5
    fig-height: 2.5
    fig-dpi: 300
    dpi: 300            # tell Pandoc the real render dpi so it sizes figures correctly
  pptx: default
execute:
  echo: true
---

## Welcome to Biostatistics

::::::: columns
:::: {.column width="60%"}
- This course teaches the whole pipeline: **raw data → clean → summarize → visualize → test → report**
- You do **not** need prior coding experience — everyone starts exactly here
- The only way to learn this is to **type the code yourself** and break it

::: callout-important
**Getting stuck is not failing — it is the job.**\
Every working scientist reads error messages and searches for answers every day.
:::
::::

:::: {.column width="40%"}
::: callout-note
**Today's roadmap**

1.  Asking a good question
2.  Organizing a project
3.  Meeting R and Positron
4.  Loading data and your first plot
:::
::::
:::::::

## Part 1 · Asking a Good Question {.divider}

## What makes something science?

:::::: columns
::: {.column width="60%"}
- Science makes **predictions** and tests them with a **falsifiable** approach — statistics
- A claim that could never be shown wrong is not testable, and not science

**Example:**

- *"Needles on the shady side of a tree are longer"* — testable ✅
- *"The forest decides needle length"* — not testable ❌
:::

:::: {.column width="40%"}
::: callout-note
**📖 New word**

**Falsifiable** = a prediction that could, in principle, be proven wrong. A good hypothesis is one you could disprove.
:::

::: callout-note
**📖 Reference**

Whitlock & Schluter, *The Analysis of Biological Data*, Ch. 1 — Statistics and Samples.\
Gotelli & Ellison, *A Primer of Ecological Statistics*, Ch. 4 — Framing and Testing Hypotheses.
:::
::::
::::::

## Two ways of reasoning toward that question

::::::: columns
:::: {.column width="50%"}
**Inductive (specific → general)**

![Flowchart of the inductive reasoning phase, highlighted at the top: Information (observation, experiment, literature) leads to a Model, which leads to Hypothesis, then a testable Prediction, then a Test of Hypothesis that loops back via Yes/No decision points.](images/inductive_phase.png){height="3in"}

You measure needles on a few trees, notice a pattern, and generalize: *"needle length seems to vary with side of tree."*

::: callout-note
**Weakness:** the pattern held in your sample — it is not guaranteed to hold everywhere.
:::
::::

:::: {.column width="50%"}
**Deductive (general → specific)**

![The same scientific-reasoning flowchart as the inductive diagram, but with the deductive phase highlighted instead: starting from Hypothesis, moving to a testable Prediction and Test of Hypothesis, showing deduction as reasoning from a general rule down to a specific testable case.](images/deductive_phase.png){height="3in"}

You start from a general rule — *"exposure affects needle length"* — and predict what a new tree should show, then check.

::: callout-note
**Weakness:** only as strong as the general rule you started from.
:::
::::
:::::::

::: callout-tip
**In practice we do both, back and forth** — notice a pattern (induction), propose a rule, then test it on new data (deduction).
:::

## Our class question

:::::: columns
:::: {.column width="60%"}
Pine needles on the **shady** side of a tree vs. the **sunny** side:

- **Question:**
- **Prediction:**
- **H₀ (null):**
- **Hₐ (alternate):**

::: callout-note
**📖 New words**

- **Question**: what is your question
- **Prediction**: what do you think will happen
- **H₀** — the "nothing is happening" claim
- **Hₐ** — the claim we suspect might be true
- Note: Hₐ does **NOT** say *which* side is longer — that would be a **prediction**, a stronger, riskier claim
:::
::::

::: {.column width="40%"}
![Photo comparing needle length and bundle count across six pine species (Pinus ponderosa, nigra, resinosa, strobus, sylvestris, banksiana), showing needles ranging from long single pairs to short clusters of five, illustrating the kind of length measurement the class's own pine-needle sampling will use.](images/pine_needles.jpg){width="2.03in"}
:::
::::::

## Designing a defensible sample

::::::: columns
:::: {.column width="60%"}
- **Can we test this with needles from one tree?**
  - No — that tree could just happen to have short needles on one side due to other drivers.
  - Treating many needles from *one* tree as if they were independent replicates is called **pseudoreplication** (Hurlbert 1984).
- **The tree — not the needle — is the unit of replication.**
  - We need needles from *many* trees, sampled the same way each time.
- **Randomize** which trees and branches you sample, so your own bias doesn't sneak into the data.

::: callout-important
**Dependent vs. independent variables**

- **Independent (X):** sun exposure — shady or sunny — what we group by
- **Dependent (Y):** needle length — what we measure
:::
::::

:::: {.column width="40%"}
::: callout-tip
**🖐 Today in the field**

Each team collects needles from the shady side **and** the sunny side of the same tree, using one agreed-on method — same height, same "mature needle" definition, same spot on the branch.

Then we come back, agree on how to measure, measure the needles, and enter the data in Excel.
:::

::: callout-note
**Because both sides come from the same tree, the two measurements are *paired*.** Keep track of which team measured which tree — we will need that structure later today, and again in Week 4.
:::

::: callout-note
**📖 Reference**

Whitlock & Schluter, Ch. 1 — Statistics and Samples, on random sampling and what makes a sample representative.\
Gotelli & Ellison, Ch. 6 — Designing Successful Field Studies, on replication and pseudoreplication.
:::
::::
:::::::

## Part 2 · Organizing Your Project {.divider}

## Data is the raw material — treat it that way

::::::: columns
:::: {.column width="60%"}
- Decide your **variable names and units before you collect anything** — this is a **controlled vocabulary**
- Example: `needle_length_mm`, not `Needle Length (mm)` — all lowercase, no spaces, no parentheses
- **Put the unit *in* the name as a suffix** (`_mm`, `_g`, `_c`) so it can never get separated from the number
- Plan where data goes the moment you collect it — do not let it pile up in email or texts

::: callout-note
**✅ Key idea**

A few minutes of naming discipline today saves hours of confusion in Wrangling Your Data, when we clean messy data.
:::
::::

:::: {.column width="40%"}
::: callout-tip
**Controlled vocabulary example**

| Bad                    | Good               |
|------------------------|--------------------|
| `Needle Length (mm)`   | `needle_length_mm` |
| `Sun Exposure`         | `side`             |
| `Team Name`            | `team`             |
:::
::::
:::::::

## One folder per project

::::::: columns
:::: {.column width="60%"}
```
pine_project/
├── data/       ← raw files, never overwritten
├── scripts/    ← your .R code
├── figures/    ← plots you save
└── output/     ← place to save modified data
```

- Open this folder as your workspace in Positron (**File → Open Folder…**)
- Every file path then becomes **relative**
  - no more `C:/Users/.../Desktop/final_FINAL2.xlsx`
  - never call `setwd()` to set the working directory — **not once, not ever**

::: callout-note
**✅ Key idea**

Keep **raw data untouched**.\
R writes new, processed files — so you can always trace your steps back to the original numbers.
:::
::::

:::: {.column width="40%"}
::: callout-note
**📖 New word**

**Metadata** = data *about* your data: who collected it, when, how, with what instrument.\
Without it, data are nearly useless — even to future-you.
:::

::: callout-note
**📖 Reference**

Gotelli & Ellison, Ch. 8 — Managing and Curating Data.

[📖 R4DS §6 — Workflow: scripts and projects](https://r4ds.hadley.nz/workflow-scripts.html)
:::
::::
:::::::

::: callout-tip
**🖐 Preview**

Tidy data — one row per observation, one column per variable — is the target we're organizing toward. We'll formalize this in Wrangling Your Data.
:::

## Part 3 · Meet R and Positron {.divider}

## R is the engine, Positron is the dashboard

:::::: columns
::: {.column width="60%"}
- **R** — a free language for data and statistics
- **Positron** — the editor (IDE) you drive it through
- Four panes you'll use constantly:

| Pane              | Where    | What it does              |
|-------------------|----------|---------------------------|
| **Editor**        | top-left | your scripts live here    |
| **Console**       | bottom   | where code actually runs  |
| **Environment**   | right    | everything you've stored  |
| **Plots / Files** | right    | figures and project files |
:::

:::: {.column width="40%"}
::: callout-note
**📖 New word**

**IDE** = Integrated Development Environment. R is the car engine; Positron is the seat, wheel, and dashboard.
:::
::::
::::::

## R as a calculator, and storing values

::::::: columns
:::: {.column width="60%"}
Type math into the **console** and press Enter:

``` r
3 + 5
12 / 7
2 ^ 10
```

To keep a value, give it a name with the **assignment operator** `<-`:

``` r
x <- 7      # store 7 under the name x
x           # read it back
x * 2       # use it in math
```

::: callout-warning
**⚠️ Watch out!**

R is case-sensitive: `x` and `X` are different objects. Use `<-`, not `=`, to assign.
:::
::::

:::: {.column width="40%"}
::: callout-note
**📖 New word**

**Object** = a named container holding a value. Watch it appear in the **Environment** pane the moment you create it.

[📖 R4DS §2 — Workflow: basics](https://r4ds.hadley.nz/workflow-basics.html)
:::
::::
:::::::

## Names, comments, and functions

::::::: columns
:::: {.column width="60%"}
- Use **`lower_snake_case`**: `needle_length`, not `x2` or `NeedleLength`
- Anything after `#` is a **comment** — a note for humans, ignored by R

``` r
# average of four needle lengths, in mm
needle_length <- c(20, 21, 23, 25)
mean(needle_length)     # a function call
```

A **function** takes **arguments** in `()` and returns a result. Stuck? Ask for help:

``` r
?mean
args(mean)
```

::: callout-tip
**🖐 Try it yourself**

Predict what these return, then run them:

``` r
round(0.5)    # you expect 1 ...
round(1.5)    # you expect 2 ...
round(2.5)    # you expect 3 ...
```

You get **0, 2, 2**. R rounds a value ending in exactly `.5` to the nearest **even** number, not always upward. This is the IEEE 754 standard ("round half to even"), and it is deliberate — it keeps rounding errors from accumulating in one direction.

If you want the school-textbook rule instead, use `janitor::round_half_up()`, which turns those into **1, 2, 3**.
:::
::::

:::: {.column width="40%"}
::: callout-note
**📖 New words**

- **Comment** — ignored by R, read by humans
- **Argument** — an input handed to a function
- **Function** — a named, reusable operation

[📖 R4DS §4 — Workflow: code style](https://r4ds.hadley.nz/workflow-style.html)
:::
::::
:::::::

## Vectors, data types, and scripts

::::::: columns
:::: {.column width="60%"}
A **vector** is a row of values made with `c()` — a spreadsheet column is really just a vector:

``` r
needle_length_mm <- c(20, 21, 23, 25)      # numeric
side             <- c("shady", "sunny")    # character — needs quotes
```

The **console forgets**. A **script** (`.R` file) remembers:

- **File → New File → R File**, save it, run lines with Ctrl/Cmd+Enter

::: callout-warning
**⚠️ Watch out!**

Without quotes, `shady` means "find an object called shady," not the text `"shady"`.\
R will error if no such object exists.
:::
::::

:::: {.column width="40%"}
::: callout-note
**✅ Key idea**

**Console** = scratch paper.\
**Script** = the lab notebook you keep and rerun.
:::
::::
:::::::

## Packages — extending what R can do

::::::: columns
:::: {.column width="60%"}
**Install once**, in the console:

``` r
install.packages("tidyverse")
```

**Load every session**, at the top of every script:

``` r
library(tidyverse)   # wrangling + ggplot2
```

::: callout-warning
**⚠️ Watch out!**

*"could not find function..."* almost always means you forgot the `library()` line for that session.
:::
::::

:::: {.column width="40%"}
::: callout-note
**📖 New words**

- **Package** — the installed toolbox (once)
- **Library** — loading it for today's session (every time)

[📖 R4DS §1 — Data visualization](https://r4ds.hadley.nz/data-visualize.html) opens with installing and loading the tidyverse.
:::
::::
:::::::

## Part 4 · Load Data and Make Your First Plot {.divider}

## Bring the data in

::::::: columns
:::: {.column width="60%"}
[📥 Download the data](data/pine_data.csv){.btn .btn-primary download="pine_data.csv"}

Put the file in `data/`, then:

```{r getting-started-01}
library(tidyverse)

pine_df <- read_csv("data/pine_data.csv")
```

::: callout-note
**✅ Key idea**

`read_csv()` comes free with `library(tidyverse)`. Your file's first row becomes the **column names** in R.
:::
::::

:::: {.column width="40%"}
::: callout-note
**Our columns**

- `team` — which team collected the tree
- `side` — `"shady"` or `"sunny"`
- `replicate` — which needle (1–8) within a team-and-side
- `needle_length_mm` — needle length in mm
- `needle_width_mm` — needle width in mm

[📖 R4DS §7 — Data import](https://r4ds.hadley.nz/data-import.html)
:::
::::
:::::::

## Look before you trust it

::::::: columns
:::: {.column width="60%"}
```{r getting-started-02}
glimpse(pine_df)
```

```{r getting-started-03}
dim(pine_df)
```

::: callout-tip
**🖐 Notice**

64 rows — but only **4 teams**, each measuring **8 needles** on **2 sides** of **one tree**. Rows are not the same thing as independent observations. Hold that thought.
:::
::::

:::: {.column width="40%"}
::: callout-warning
**⚠️ Watch out!**

If a number column shows as `<chr>` in `glimpse()`, a stray letter or comma is hiding in that column.
:::
::::
:::::::

## Your first plot: `ggplot`, built in layers

::::::: columns
:::: {.column width="60%"}
Every `ggplot` needs three things, joined with `+`:

1.  **data** — which data frame
2.  **aes()** — which columns map to x and y
3.  **geom** — how to draw it

```{r getting-started-04}
#| fig-width: 2
#| fig-height: 2
#| fig-alt: "Scatterplot of pine needle length in millimeters for the shady and sunny sides of the tree, with individual points shown for each side."
ggplot(pine_df, aes(x = side, y = needle_length_mm)) +
  geom_point()
```

::: callout-warning
**⚠️ Watch out!**

`+` sits at the **end** of a line, never the start.\
this is what adds layers to the plot and order matters
:::
::::

:::: {.column width="40%"}
::: callout-note
**✅ Key idea**

*Seeing* your data is the single most important habit in this course — before you summarize or test anything, plot it.
:::

::: callout-note
**📖 Reference**

Whitlock & Schluter, Ch. 2 — Displaying Data.

[📖 R4DS §1 — Data visualization](https://r4ds.hadley.nz/data-visualize.html)
:::
::::
:::::::

## One layer at a time

::::::: columns
:::: {.column width="60%"}
```{r getting-started-05}
#| fig-width: 3.2
#| fig-height: 2.2
#| fig-alt: "Boxplot of needle length in millimeters for shady versus sunny sun exposure, with individual jittered points in orange-red overlaid on each box to show the underlying data."
ggplot(pine_df, aes(x = side, y = needle_length_mm)) +
  geom_boxplot() +
  geom_jitter(width = 0.15, alpha = 0.6, color = "tomato") +
  labs(x = "Sun exposure",
       y = "Needle length (mm)",
       title = "Needle length by sun exposure")
```

::: callout-tip
**🖐 Try it yourself**

Swap `side` for `team`. That plot answers a different question — *how much do the four teams differ from each other?* Keep the answer in mind for the next slide.
:::
::::

:::: {.column width="40%"}
::: callout-note
**✅ Key idea**

A boxplot shows the summary; the jittered points show every real value underneath it. Show both when you can.
:::
::::
:::::::

## Part 5 · 64 Rows, But How Many Observations? {.divider}

## The tempting (wrong) picture

::::::: columns
:::: {.column width="60%"}
The boxplot you just made pooled all **64 needles** as if each were an independent measurement:

```{r getting-started-06}
#| fig-width: 3.2
#| fig-height: 2.2
#| fig-alt: "Boxplot of needle length by shady versus sunny side, pooling all 64 needles, with jittered points colored by team showing that the four teams occupy visibly different bands of needle length."
ggplot(pine_df, aes(x = side, y = needle_length_mm)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(color = team), width = 0.15, alpha = 0.8) +
  labs(x = "Sun exposure", y = "Needle length (mm)", color = "Team")
```

Colour by `team` and the problem jumps out: the four teams sit in **different bands**. A needle is not independent of the other seven from the same tree.
::::

:::: {.column width="40%"}
::: callout-important
**⚠️ Pseudoreplication**

We have **n = 4 trees**, not n = 64 needles.\
Counting needles inflates our confidence in a difference we have barely measured.
:::
::::
:::::::

## Doing it the right way: `group_by()` + `summarise()`

::::::: columns
:::: {.column width="60%"}
Collapse each team-and-side down to **one number** — the thing we actually replicated:

```{r getting-started-07}
team_means <- pine_df |>
  group_by(team, side) |>
  summarise(mean_length_mm = mean(needle_length_mm),
            .groups = "drop")

team_means
```

64 rows became **8** — four teams × two sides. *This* is the dataset our question is really about.
::::

:::: {.column width="40%"}
::: callout-note
**📖 New words**

- **`group_by()`** — split the data into groups
- **`summarise()`** — collapse each group to one row
- **Pipe `|>`** — send the result of one step into the next

We'll go deep on all three in **Wrangling Your Data**.

[📖 R4DS §3 — Data transformation](https://r4ds.hadley.nz/data-transform.html)
:::
::::
:::::::

## The two pictures side by side

::::::: columns
:::: {.column width="60%"}
```{r getting-started-08}
#| fig-width: 3.4
#| fig-height: 2.4
#| fig-alt: "Line-and-point plot of the four team mean needle lengths, with each team's shady and sunny means joined by a line, showing that every team's sunny mean is higher than its shady mean even though the teams differ widely in overall needle length."
ggplot(team_means, aes(x = side, y = mean_length_mm, group = team)) +
  geom_line(aes(color = team), linewidth = 0.8) +
  geom_point(aes(color = team), size = 2) +
  labs(x = "Sun exposure", y = "Mean needle length (mm)", color = "Team")
```

Each line is **one tree**. The teams are far apart vertically — but **every single line slopes the same way**.
::::

:::: {.column width="40%"}
::: callout-note
**✅ Key idea**

Because both sides come from the same tree, these are **paired** measurements. Comparing each tree *to itself* cancels out the big between-team differences.
:::

::: callout-tip
**🖐 Coming in Week 4**

We'll put numbers on this — and see how a **paired t-test** finds a signal that the naive test on 64 needles misses.
:::
::::
:::::::

## Save your plot

::::::: columns
:::: {.column width="60%"}
```{r getting-started-09}
#| eval: false
needle_plot <- ggplot(pine_df, aes(x = side, y = needle_length_mm)) +
  geom_boxplot() +
  geom_jitter(width = 0.15, alpha = 0.6, color = "tomato") +
  labs(x = "Sun exposure", y = "Needle length (mm)")

ggsave("figures/needle_length.png",
       plot = needle_plot,
       width = 3, height = 3, units = "in", dpi = 300)
```

::: callout-warning
**⚠️ Watch out!**

`ggsave()` wants the **filename first**, then `plot =`. Always use `dpi = 300` — print/poster quality.
:::
::::

:::: {.column width="40%"}
::: callout-note
**📖 File format tip**

Save figures as **PNG** for reports and submissions.
:::
::::
:::::::

## Wrap-up

::::::: columns
:::: {.column width="60%"}
Today you:

- Turned a question into a testable **H₀ / Hₐ**
- Organized a project folder with **data/scripts/figures**
- Ran your first R code — objects, functions, vectors, packages
- Loaded real data and made — and saved — your **first `ggplot`**
- Saw why **64 rows is not 64 observations**, and collapsed them to the 4 trees we actually replicated

::: callout-tip
**🖐 Before next class**

Finish the worksheet: load `pine_data.csv`, run `glimpse()`, and make one more plot — `needle_width_mm` by `side` this time. Save it to `figures/`.
:::
::::

:::: {.column width="40%"}
::: callout-note
**Up next — Wrangling Your Data**

- `filter()`, `select()`, `mutate()`, `arrange()`
- Cleaning a genuinely messy version of this dataset
- The pipe `|>` and building a pipeline
:::
::::
:::::::

## Readings

::::::: columns
:::: {.column width="60%"}
**Before next class** — both chapters are posted as PDFs on **Canvas**:

- **Whitlock & Schluter**, *The Analysis of Biological Data* — **Ch. 1**, Statistics and Samples (populations vs. samples, why we sample, sampling error)
- **Gotelli & Ellison**, *A Primer of Ecological Statistics* — **Ch. 8**, Managing and Curating Data (raw data, metadata, project organization)

**If you want more on the science and hypothesis side:**

- Whitlock & Schluter, Ch. 2 — Displaying Data
- Gotelli & Ellison, Ch. 4 — Framing and Testing Hypotheses
::::

:::: {.column width="40%"}
::: callout-note
**📖 R for Data Science (2e)**

Free online at [r4ds.hadley.nz](https://r4ds.hadley.nz/) — the R side of today:

- [§1 — Data visualization](https://r4ds.hadley.nz/data-visualize.html)
- [§2 — Workflow: basics](https://r4ds.hadley.nz/workflow-basics.html)
- [§6 — Workflow: scripts and projects](https://r4ds.hadley.nz/workflow-scripts.html)
- [§7 — Data import](https://r4ds.hadley.nz/data-import.html)
- [§8 — Workflow: getting help](https://r4ds.hadley.nz/workflow-help.html)
:::
::::
:::::::

## Getting unstuck

When code breaks — and it will, that is normal:

1.  Read the **error message** out loud; it usually names the line
2.  Check the usual suspects: `library(tidyverse)` loaded? Spelling? A missing `)` or `+` at the *start* of a line?
3.  `?function_name` opens the help page
4.  Bring the **exact error** (copy-paste it) to class or office hours

[📖 R4DS §8 — Workflow: getting help](https://r4ds.hadley.nz/workflow-help.html) shows how to write a question others can actually answer.

::: callout-note
**✅ Key idea**

Every working scientist googles error messages daily. Getting stuck is not failing — it is the job.
:::
