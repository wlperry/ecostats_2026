...
Open Excel and make **row 1** your column headers — nothing else goes in that row. Use the same **controlled vocabulary** your group already agreed on in Part 1, written the `lower_snake_case` way (no spaces, no units hidden in the text):

| Column      | What goes in it                                 |
|-------------|-------------------------------------------------|
| `date`      | the date you collected, one format for everyone |
| `group`     | your field team's name                          |
| `n_s`       | `n` or `s` — which compass side                 |
| `sun`       | `shady` or `sunny` — light exposure             |
| `tree_no`   | which tree, as a number                         |
| `length_mm` | the needle length, in millimeters, numbers only |
...
✏️ **Your turn:** How many rows should your sheet have, if your group measured `___` needles per side, per tree, on `___` trees? Show your arithmetic: `________________________`
...
✏️ **Your turn:** What are the two file names you just saved, and are they both sitting in `pine_project/data/`? `________________________`
