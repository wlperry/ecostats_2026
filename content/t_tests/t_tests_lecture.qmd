...
description: "The hypothesis-testing framework, the t-distribution, and one-sample, two-sample (Student's and Welch's), and paired t-tests — with assumption checks — on the pine needle data the class collected."
...
## Setup — Loading Our Pine Needle Data
...
For two sample T-tests, degrees of freedom = n₁ + n₂ − 2. Once we average the needles up to one value per tree side (next slide), we have **4 trees per side** — so df = 4 + 4 − 2 = **6**. Note that n is the number of *trees*, not needles.
:::
::::
...
This is the data **you** collected: `team` is your field team (one tree each), `side` is sunny vs. shady, `replicate` is which of the 8 needles, and `needle_length_mm` is the measurement.
...
::: callout-important
**Why average first?** The true experimental unit here is *tree side*, not *needle*. Treating every needle as independent would inflate our sample size and our confidence — the same pseudoreplication trap from Getting Started.

This is exactly the **two-stage summary** from Describing Your Data: `group_by()` down to the unit you actually replicated, *then* analyse. Everything from here on runs on one number per tree side.
:::
::::
...
stats_df <- p_df %>%
  ...
  summarize(
    ...
    se_length   = sd(needle_length_mm, na.rm = TRUE) / sum(!is.na(needle_length_mm))^.5,
    count       = sum(!is.na(needle_length_mm)),
    .groups = "drop"
  )
stats_df
```
...
#| message: false
#| warning: false
#| fig-alt: "Boxplot of mean needle length by side of tree (shady vs. sunny), with one point per field team and a line joining each team's shady and sunny tree means, showing that the two boxes overlap heavily while every team's line slopes the same way."
p_plot <- p_df %>%
  ggplot(aes(side, needle_length_mm, fill = side)) +
  geom_boxplot(alpha = 0.7) +
  geom_point(aes(group = team, color = team)) +
  geom_line(aes(group = team, color = team)) +
  labs(x = "Side of tree", y = "Mean needle length (mm)",
       caption = "One point per team's tree") +
...
The boxes overlap a lot. But **every line slopes the same way.** Hold that thought — it is the whole story of today's last section.
:::
:::
:::::
...
*Worked example for reading the table — n = 8, df = 7:*

- n = 8 (df = 7) — 95% of the observations found to the left
- t = 1.895 (5% are outside)

(Our own data has 4 trees per side, so df = 3 for a paired test and df = 6 for a two-sample test. Same table, different row.)
...
**By hand, for an example Sample A** (mean 17.6, s = 2.51, n = 8, so df = 7 — the row highlighted in the table):

- 95% CI = 17.6 ± 2.365 × (2.51 / 8\^0.5) = ± 2.10
- The 95% CI is between 15.50 and 19.70

**Now our own shady-side trees** (n = 4, so df = 3, and the critical t is much larger):
...
cat("n =", n_shady, " t crit =", round(t_crit, 3), "\n")
cat("95% CI:", round(mean_shady - t_crit * se_shady, 2),
    "to",     round(mean_shady + t_crit * se_shady, 2), "mm\n")
```

::: callout-warning
**⚠️ Four trees is a *wide* interval**

With df = 3 the critical t is **3.182**, not 1.96. Small n costs you twice: a bigger multiplier *and* a bigger SE.
:::
::::
...
Run it, report it, but lean on the QQ plot, the biology, and the design — not on a p-value from four points.
:::
...
# TWO-tailed test (Ha: mu != 15), so alpha is split between the two tails
t_crit <- qt(1 - alpha/2, df)

x <- seq(-4, 6, length.out = 1000)
null_y <- dt(x, df)

hyp_data <- data.frame(x = x, y = null_y)
...
rejection_region_lower <- data.frame(
  x = seq(-4, -t_crit, length.out = 100),
  y = dt(seq(-4, -t_crit, length.out = 100), df)
)
...
hyp_plot <- ggplot(hyp_data, aes(x = x, y = y)) +
  ...
  geom_area(data = rejection_region_lower, aes(x = x, y = y),
            fill = "red", alpha = 0.3, inherit.aes = FALSE) +
...
# Rejection regions run OUTWARD from the critical values to the edges of the
# plot -- not inward toward the hypothesized mean.
rejection_upper <- data.frame(
  x = seq(critical_value_upper, 20, length.out = 100),
  y = dt((seq(critical_value_upper, 20, length.out = 100) - hypothesized_mean)/(sample_sd/sqrt(sample_size)), df) * (sample_sd/sqrt(sample_size))
)

rejection_lower <- data.frame(
  x = seq(10, critical_value_lower, length.out = 100),
  y = dt((seq(10, critical_value_lower, length.out = 100) - hypothesized_mean)/(sample_sd/sqrt(sample_size)), df) * (sample_sd/sqrt(sample_size))
)
...
needle_mean_se_plot <- ggplot(p_df, aes(x = side, y = needle_length_mm, color = side)) +
...
::: {.column width="40%"}
```{r t-tests-20}
t_test_result <- t.test(needle_length_mm ~ side, data = p_df, var.equal = FALSE)
...
#| fig-width: 3.4
#| fig-height: 3.2
#| fig-alt: "T-distribution curve with 6 degrees of freedom for the two-sample needle-length test, red rejection regions in both tails beyond the critical t-values of plus and minus 2.447, and a dashed blue line marking the observed t-statistic of -0.855, which falls well inside the light blue non-rejection region."
# Drive the figure from the ACTUAL test so the labels can never drift
student_tt <- t.test(needle_length_mm ~ side, data = p_df, var.equal = TRUE)
...
t_dist_df <- data.frame(x = seq(-10, 10, by = 0.01)) %>%
  mutate(density = dt(x, df = obs_df))

critical_t <- qt(0.975, df = obs_df)
...
t_plot <- ggplot(t_dist_df, aes(x = x, y = density)) +
  ...
  geom_vline(xintercept = -critical_t, color = "red", linewidth = 1) +
  geom_vline(xintercept = obs_t, color = "blue", linetype = "dashed", linewidth = 1) +
  annotate("text", x = critical_t, y = 0.15,
           label = paste("Critical t =", round(critical_t, 3)), hjust = -0.1, color = "red") +
  annotate("text", x = obs_t, y = 0.25,
           label = paste("Observed t =", round(obs_t, 3)), hjust = -0.1, color = "blue") +
  annotate("text", x = 0, y = 0.2,
           label = paste("p =", signif(obs_p, 3)), size = 4) +
  labs(title = "T-Distribution with Observed T-Value",
       subtitle = paste0("Two-Sample T-Test for Needle Length (df = ", obs_df, ")"),
       x = "t-value", y = "Density") +
  theme_minimal(base_size = 9) +
...
`t.test(shady, sunny, paired = TRUE)` computes **shady − sunny**. Reverse the arguments and `t` flips sign — the p-value is identical, but the story you tell about direction is backwards. Always state which way you subtracted.
:::
...
# so the two t-statistics are directly comparable
...
:::::: columns
:::: {.column width="60%"}
Look carefully at what just happened — **both tests used exactly the same 8 numbers**, and both measured shady − sunny:
...
- The **two-sample** test throws the pairing away. Tree-to-tree variation (some teams' trees just have longer needles than others) lands in the *noise*, swamping the signal.
- The **paired** test looks only at the shady−sunny difference *within each tree*, so tree-to-tree variation cancels out entirely.

Notice the paired test has **half the degrees of freedom** (3 vs 6) — it is working with 4 differences, not 8 values. It still does far better, because the noise it removed mattered more than the df it gave up.

::: callout-note
**This is about sensitivity, not about 0.05**

Neither test crosses α = 0.05, so neither "wins" by the usual rule. That is not the point. The point is that **the correct test extracted six times more evidence from the identical measurements** — and with only 4 trees, that is the difference between a study that could detect this effect and one that never could.
:::

::: callout-important
**These data are paired by design** — each team measured both sides of the *same* tree. The paired test is the correct one here. The two-sample test is not just less powerful, it is answering a question our design never asked.
:::
::::
...
#| fig-width: 3
#| fig-height: 2.6
#| fig-alt: "Slope plot showing each team's tree as a line connecting its shady mean to its sunny mean; all four lines slope upward from shady to sunny, showing a consistent within-tree effect even though the two groups overlap heavily overall."
p_df %>%
  ggplot(aes(side, needle_length_mm, group = team)) +
  geom_line(color = "grey50") +
  geom_point(aes(color = side), size = 2) +
  labs(x = "Side of tree", y = "Length (mm)",
       title = "Each line = one tree",
       caption = "All four trees shift the same way") +
  theme_minimal(base_size = 9) +
  theme(legend.position = "none")
```
:::
