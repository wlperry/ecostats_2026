...
::: callout-note
**🔮 Predict first:** `n_s == "n" | length_mm > 22` keeps a row if the side is lee **or** the needle is longer than 22mm. Will this ever keep a *windward* needle? Under what condition?
:::

:::::: columns
::: {.column width="60%"}
```{r}
# Lee-side needles, OR any exceptionally long needle
pine_csv_df |>
...
`|` is the pipe **character**, not the pipe **operator** `|>`. They look related but do completely different jobs — one is logical OR, the other chains steps together.
:::
::::
::::::
...
`x %in% c(a, b, c)` asks "is `x` equal to *any* of these?" — it's the clean way to write an OR across many possible values without a long chain of `|`.
:::
::::
::::::
...
`is.na(x)` is itself a boolean test — `TRUE`/`FALSE` — so it plugs into `filter()` exactly like `==` or `>` does.
:::
::::
...
`pine_csv_df` has no missing values, so `is.na()` has nothing to catch here. Module 3 digs into *why* missing values matter so much once you're computing a sample size or a mean.
:::
::::
:::::::
...
`case_when()` checks conditions **top to bottom** and uses the *first* one that's `TRUE`. Once a row matches `length_mm < 16`, R never even looks at the later conditions for that row.
:::
::::
::::::
