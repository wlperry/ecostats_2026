...
lion_data <- lion_data %>%
  mutate(predicted = predict(lion_model))
...
ant_model <- lm(ant_spp ~ elevation + latitude, data = ant_df)
...
grid$ant_spp <- predict(ant_model, newdata = grid)
...
plot(ant_model, which = 1)
plot(ant_model, which = 2)
par(mfrow = c(1, 1))
```
::::
:::::::
...
- All predictors: `avPlots(ant_model)`
- One predictor: `avPlot(ant_model, variable = "elevation")`
...
| Predictor | Alone | Controlling for the other |
|-----------|-----------|-----------|
| elevation | −0.0146 | **−0.0123** |
| latitude | −2.340 | **−2.010** |

Both shrink a little once the other predictor is in the model — each was taking a small amount of credit that belonged to the other.

::: callout-note
**✅ Why only "a little" here?**

Because elevation and latitude are **nearly independent** in these data (r = 0.18). When predictors *are* strongly correlated, the same comparison can change a slope's size dramatically — or even flip its sign.
:::
::::

:::: {.column width="60%"}
```{r multiple-regression-08}
#| echo: false
#| message: false
#| warning: false
#| paged-print: false
...
$$VIF_{X_j} = \frac{1}{1 - R^2_j}$$

where $R^2_j$ is the R² you get from regressing **$X_j$ on all the *other* predictors** — not the pairwise correlation between two of them. (With only two predictors those happen to coincide; with three or more they do not.)

- Rule of thumb: **VIF > 10 is bad**; some authors use > 5
...
```{r multiple-regression-10}
# car::vif() does this for you
vif(ant_model)
```
::::
...
Both VIFs are about **1.03** — essentially no inflation. Latitude and elevation are nearly independent here, which is why each partial slope is interpretable on its own.

A VIF of 1 means "this predictor shares nothing with the others." That is the ideal, and it is rarer than you would like.
:::
::::
:::::::
...
- Extended the regression line to a **plane** — two or more predictors at once
- Learned to read a **partial slope**: the effect of one predictor *holding the others constant*, which is not the same as its slope on its own
- Tested the whole model with an **overall F**, and single predictors with **partial F-tests**
- Diagnosed **collinearity** with `vif()`, and saw why a high VIF poisons the individual slopes without necessarily hurting the fit
- Fitted an **interaction** and paid for it in degrees of freedom and interpretability
- Coded a categorical predictor as **dummy variables** against a reference level
- Compared models with **adjusted R²** and **AIC**, which penalize you for adding predictors
- Ranked predictors by partial F, partial R², and standardized slopes

::: callout-important
**⚠️ The trap to remember**

Adding predictors **always** raises plain R². That is why we never compare models on R² alone — adjusted R² and AIC exist precisely because "it explains more variance" is not the same as "it is a better model."
:::
::::
...
A partial slope answers a *conditional* question — "what does elevation do among sites at the same latitude?" Change which other predictors are in the model and you have changed the question, so the number is allowed to change too.
:::

::: callout-note
**Up next — Analysis of Variance**

- What happens when every predictor is categorical
- Why ANOVA and regression are the same machinery wearing different clothes
:::
::::
:::::::
...
2.  Check the usual suspects: is it `y ~ x1 + x2` (additive) or `y ~ x1 * x2` (interaction)? Is `car` loaded for `vif()`? Are you comparing models fitted to the **same rows** — an `NA` in one predictor silently drops that row
