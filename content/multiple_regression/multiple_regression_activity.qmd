...
>
> **Every grey `r` box below is code you type into an R script — not into the Console.** Before you start: **File → New File → R Script**, and save it as `11_multiple_regression.R` in your project's `scripts/` folder. Then work down the worksheet, adding each block to the bottom of that one script, in order.
>
...
✏️ **Your turn:** Run `glimpse(forest_df)`. Which columns look continuous, and which look categorical?
...
``` r
# ── Part 2.1 · Exploring Multicollinearity ──────────────────────────────
num_vars <- forest_df %>%
...
``` r
# ── Part 2.2 · Exploring Multicollinearity ──────────────────────────────
forest_df %>%
...
✏️ **Your turn:** From the correlation matrix, which two predictors look most strongly correlated with each other?
...
``` r
# ── Part 2.3 · Exploring Multicollinearity ──────────────────────────────
...
``` r
# ── Part 2.4 · Exploring Multicollinearity ──────────────────────────────
...
✏️ **Your turn:** Which predictor(s) have VIF > 10? Does that match what you flagged from the correlation matrix?
...
``` r
# ── Part 2.5 · Exploring Multicollinearity ──────────────────────────────
...
``` r
# ── Part 3.1 · Addressing Multicollinearity ─────────────────────────────
...
✏️ **Your turn:** Did removing `season` fix the VIF problem?
...
``` r
# ── Part 4.1 · Checking the Shape of Relationships ──────────────────────
par(mfrow = c(2, 3))
...
``` r
# ── Part 4.2 · Checking the Shape of Relationships ──────────────────────
forest_df <- forest_df %>%
...
✏️ **Your turn:** Compare the AV plot for `age` before and after the log transform. Does the relationship look straighter now?
...
✏️ **Your turn:** Do the residual plots for `model_4` look better than for `model_3`? What specifically improved (or didn't)?
...
``` r
# ── Part 6.1 · Model Simplification and Comparison ──────────────────────
...
``` r
# ── Part 6.2 · Model Simplification and Comparison ──────────────────────
...
✏️ **Your turn:** Does `model_5` have a lower AIC than `model_4`? Given the "penalize complexity" logic from lecture, what does that tell you about dropping `precip`?
...
✏️ **Your turn:** Do the `sensemakr` partial R² values roughly match your by-hand `Partial_R_squared` column?
...
✏️ **Your turn:** In `model_std`, which predictor has the largest absolute standardized coefficient? Is that the same predictor that had the highest partial R² in Part 7?
...
- [ ] The question
- [ ] Your hypotheses
- [ ] Which model/transformation you used, and your justification for choosing it
