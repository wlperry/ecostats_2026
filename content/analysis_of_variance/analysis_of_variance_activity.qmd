...
>
> **Every grey `r` box below is code you type into an R script — not into the Console.** Before you start: **File → New File → R Script**, and save it as `12_analysis_of_variance.R` in your project's `scripts/` folder. Then work down the worksheet, adding each block to the bottom of that one script, in order.
>
...
✏️ **Your turn:** What is the F-statistic and p-value? At α = 0.05, do we reject H₀?
...
``` r
# ── Part 2.1 · Assumptions and Diagnostics ──────────────────────────────
par(mfrow = c(2, 2))
plot(model_aov)
par(mfrow = c(1, 1))
```
...
``` r
# ── Part 2.2 · Assumptions and Diagnostics ──────────────────────────────
...
``` r
# ── Part 2.3 · Assumptions and Diagnostics ──────────────────────────────
...
``` r
# ── Part 2.4 · Assumptions and Diagnostics ──────────────────────────────
shapiro_test <- shapiro.test(residuals(model_aov))
shapiro_test
```

✏️ **Your turn:** Do these three groups pass both the normality and equal-variance checks?
...
``` r
# ── Part 3.1 · Estimated Marginal Means and Post-Hoc Testing ────────────
...
``` r
# ── Part 3.2 · Estimated Marginal Means and Post-Hoc Testing ────────────
group_means <- c_df %>%
  group_by(treatment) %>%
  summarize(
...
✏️ **Your turn:** Is `difference` zero (or extremely close to it, within rounding error) for every row?
...
``` r
# ── Part 3.3 · Estimated Marginal Means and Post-Hoc Testing ────────────
...
``` r
# ── Part 3.4 · Estimated Marginal Means and Post-Hoc Testing ────────────
...
``` r
# ── Part 3.5 · Estimated Marginal Means and Post-Hoc Testing ────────────
...
``` r
# ── Part 3.6 · Estimated Marginal Means and Post-Hoc Testing ────────────
plot(emmeans_df)
...
✏️ **Your turn:** Which two treatments share a compact-letter-display letter (meaning they're *not* significantly different)?
...
``` r
# ── Part 3.7 · Estimated Marginal Means and Post-Hoc Testing ────────────
# Write your code in the R script:
```
:::
...
✏️ **Your turn:** Do the Shapiro-Wilk and Levene's tests confirm this data violates ANOVA's assumptions?
...
✏️ **Your turn:** Does the parametric ANOVA's conclusion on the violated data agree with the Kruskal-Wallis conclusion? If they disagree, which would you trust more, and why?
