...
p1_anova_plot <- ggplot(circ_data, aes(x = treatment, y = phase_shift, color = treatment)) +
  ...
  geom_point(aes(x = circ_data$treatment, y = circ_data$phase_shift, color = treatment),
             size = 2, position = position_jitter(width = 0.15)) +
...
**Interpretation:** H = test statistic (χ² approximation); df = k − 1 = 2; **p = 0.009** (significant at α = 0.05); conclusion: at least one group differs.

**Compare to parametric ANOVA:**

```{r analysis-of-variance-32}
#| echo: false
anova(circ_model)
```
::::
...
| Data | ANOVA (parametric) | Kruskal-Wallis |
|------------------|--------------------|------------------|
| Original | F = 7.29, **p = 0.004** ✅ | H = 9.42, **p = 0.009** ✅ |
| Violated | F = 2.84, **p = 0.084** ❌ | H = 6.85, **p = 0.033** ✅ |

::: callout-important
**The parametric test lost the effect. The rank test kept it.**

Nothing about the biology changed — we only inflated one group's variance and pushed two points out into the tail. That was enough to drag the F-test's p-value from 0.004 to 0.084, across the α = 0.05 line.

Kruskal-Wallis barely noticed, because ranks don't care *how far* out an outlier is, only that it is out there.
:::
::::

:::: {.column width="40%"}
```{r analysis-of-variance-37}
#| echo: false
#| fig-width: 3.2
#| fig-height: 2.6
...
**Interpretation of p-values:** Control vs Eyes: p = 0.004 (significant); Control vs Knees: p = 1.000 (not significant); Eyes vs Knees: p = 0.079 (not significant).
...
```{r analysis-of-variance-40}
#| echo: false
# Pull the numbers straight out of the two test objects, so this summary
# can never drift away from the output printed above it.
grab <- function(res) {
  p <- res$p.value
  c(p["Eyes", "Control"], p["Knees", "Control"], p["Knees", "Eyes"])
}

tibble(
  comparison    = c("Control vs Eyes", "Control vs Knees", "Eyes vs Knees"),
  original_data = round(grab(pairwise_original_result), 4),
  violated_data = round(grab(pairwise_violated_result), 4)
)
```
::::
:::::::
...
**Example write-up:** "A Kruskal-Wallis test was conducted to compare the effect of light treatment on circadian rhythm phase shift across three groups: Control (n = 8), Knees (n = 7), and Eyes (n = 7). Median phase shifts were −0.48 hours (IQR = 0.89) for Control, −0.29 hours (IQR = 0.93) for Knees, and −1.48 hours (IQR = 0.68) for Eyes. The test revealed a statistically significant difference in phase shift between the groups, H(2) = 9.42, p = 0.009, ε² = 0.45. Post-hoc pairwise comparisons using Dunn's test with Bonferroni correction indicated that the Eyes treatment differed significantly from both Control (p = 0.016) and Knees (p = 0.032), while Control and Knees did not differ (p = 1.000)."

::: callout-note
**📖 Where ε² comes from**

$$\varepsilon^2 = \frac{H}{(n^2-1)/(n+1)}$$

Here $9.42 / 21 = 0.45$ — the rank-based analogue of R². Watch out: some software reports $\eta^2_H = (H-k+1)/(n-k)$ instead, which gives 0.39 for these data. **Say which one you used.**
:::
::::
...
> The Kruskal-Wallis test revealed significant differences between treatment groups, H(2) = 9.42, p = .009.
...
# Read H and p straight off the test object rather than typing them in,
# so the caption can never disagree with the analysis.
cat("Table 1. Descriptive Statistics for Phase Shift by Treatment\n\n",
    sprintf("\n\nNote. IQR = interquartile range.\nKruskal-Wallis H(%d) = %.2f, p = %.3f\n",
            kruskal_original_result$parameter,
            kruskal_original_result$statistic,
            kruskal_original_result$p.value))
...
- Saw that **ANOVA *is* regression** with a categorical predictor — same partitioning, same F-ratio, different-looking output
- Split $SS_{total}$ into $SS_{among}$ and $SS_{within}$, and turned them into mean squares
- Built the **F-ratio** as signal ÷ noise, and confirmed that with two groups **F = t²** exactly
- Checked the assumptions — Levene's for equal variance, Shapiro-Wilk on the **residuals**, and the four diagnostic plots
- Used **Tukey's HSD** to find *which* groups differ, after the omnibus test said *some* group does
- Reported the result the way a journal expects, with an effect size
- Switched to **Kruskal-Wallis** and **Dunn's test** when assumptions fail

::: callout-important
**⚠️ The order matters**

A significant F says *at least one* group differs — it does not say which. Post-hoc tests answer that, and they only make sense **after** a significant omnibus test. Running all pairwise t-tests instead inflates your Type I error with every comparison.
:::
::::
...
We deliberately broke the assumptions and watched the parametric p-value slide from **0.004 to 0.084** while the rank-based one held at **0.033**.

The data's biology never changed. Only its shape did. That is what "assumption violation" costs you, in one number.
:::

::: callout-note
**Up next — Two-Way ANOVA**

- Two categorical predictors at once
- Interaction: when the effect of one factor depends on the other
:::
::::
:::::::
...
2.  Check the usual suspects: is your grouping variable a **factor** (`as.factor()`)? Did you pass the model to `TukeyHSD()` via `aov()`, not `lm()`? Is `car` loaded for `leveneTest()`, `FSA` for `dunnTest()`?
