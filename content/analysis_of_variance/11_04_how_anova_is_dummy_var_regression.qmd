...
```{r plot-raw-data}
#| fig-alt: "Jittered scatterplot of plant height by fertilizer type (A, B, C), with individual data points in black and each group's mean height marked as a red dot. Mean height increases from fertilizer A to B to C."
...
```{r plot-coefficients}
#| fig-alt: "Bar chart of the three regression coefficients: the intercept (Group A mean, about 10 cm), the Group B minus Group A difference (about 6 cm), and the Group C minus Group A difference (about 12 cm), each labeled with its value."
coef_data <- tibble(
...
```{r equivalence-plot}
#| fig-alt: "Grouped bar chart comparing the ANOVA group mean and the regression-predicted value for each fertilizer type (A, B, C), with value labels above each bar. The ANOVA mean and regression prediction bars are equal height within each fertilizer group, showing the two methods produce identical values."
