source('GetDetectionMatrix.R')
source('GetSiteCovariates.R')

nrepls = 50   # number of replicates per sample site

  #  get data for Forest sites
  Ymat = GetDetectionMatrix(habitat='Forest')
  nsites = dim(Ymat)[2]
  n = dim(Ymat)[1]
  nzeros = 75 - n  #  75 = max species richness
  ...
  ## #  get data for Bog sites
  ## Ymat = GetDetectionMatrix(habitat='Bog')
  ## nsites = dim(Ymat)[2]
  ## n = dim(Ymat)[1]
  ## nzeros = 25 - n  #  25 = max species richness
  ## Xmat = GetSiteCovariates()
  ## x1 = as.vector(Xmat[,'lat'])
  ## x1 = (x1 - mean(x1))/sd(x1)
  ## x2 = as.vector(Xmat[,'area'])
  ## x2 = (x2 - mean(x2))/sd(x2)
  ## x3 = as.vector(Xmat[,'elev'])
  ## x3 = (x3 - mean(x3))/sd(x3)
  ## X = cbind(x1, x2, x3)
  ## ncovs = dim(X)[2]
  ## # augment data matrix with all-zero row vectors
